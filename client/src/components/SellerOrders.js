...
function SellerOrders() {
    ...
    const handleVerifyPayment = async (orderId) => {
        try {
            await axios.put(`http://localhost:5000/api/payments/${orderId}/verify`, {}, {
                headers: { Authorization: `Bearer ${user.token}` }
            });
            fetchOrders();
    ...
    const handleShipping = async (orderId) => {
        try {
            await axios.put(
                `http://localhost:5000/api/orders/${orderId}/shipped`,
                {},
                { headers: { Authorization: `Bearer ${user.token}` } }
            );
            fetchOrders();
                        ...
                        <div key={order.id} className="p-4 bg-gray-50 rounded-lg shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    {order.payment_proof && (
                                        <a href={`http://localhost:5000${order.payment_proof}`} target="_blank" rel="noreferrer" className="text-sky-600 hover:underline">ดูหลักฐานการชำระเงิน</a>
                                    )}
                                </div>
                            </div>
